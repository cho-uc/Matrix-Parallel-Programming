...
//###############################################################
	...
	//___________________________________
		...
		for(int i = 0; i < matrix_size; ++i){
			delete[] U_parallel[i]; //deletes an inner array 
			delete[] U_series[i];
			delete[] L_parallel[i];
			delete[] L_series[i];
		}
	...
	for(int i = 0; i < matrix_size; ++i){
		delete[] matrix_decompose.first[i];  //deletes an inner array 
	...
	for(int sample=0;sample<no_of_data ;++sample){	
		...
		int count_error=0;//Re-initialization
		count_error=countError(U_series,U_parallel);
		cout << error_sum<<"           "<< count_error<<endl;
	...
	*/
	...
	for(int sample=0;sample<no_of_data ;++sample){	
		int i, j, k;
		vector< vector<float> > a_series= a;
		vector< vector<float> > a_parallel= a;
		...
		int count_error=0;//Re-initialization
		count_error=countError(a_series,a_parallel);
		cout << error_sum<<"           "<< count_error<<endl;
